 ...
 * @LastEditTime: 2020-09-12 15:15:11
...
function CRUD (options) {
  ...
  const methods = {
    ...
    toAdd () {
      ...
      crud.states.add = CRUD.STATUS.PREPARED
    ...
    toEdit (data) {
      ...
      crud.states.edit = CRUD.STATUS.PREPARED
    ...
    doEdit () {
      ...
      crud.crudMethod
        ...
        .then(() => {
          ...
          crud.getDataStatus(crud.getDataStatus(crud.form)).edit =
            CRUD.STATUS.NORMAL
          crud.editSuccessNotify()
          crud.resetForm()
          crud.callVmHook(crud, CRUD.HOOK.afterSubmit)
          crud.refresh()
        })
        .catch(() => {
    ...
    getQueryParams: function () {
      ...
      Object.key(crud.params).length !== 0 &&
        Object.key(crud.params).forEach(item => {
    ...
    attchTable () {
      ...
      table.$on('expand-change', (row, expanded) => {
        ...
        const children = lazyTreeNodeMap[crud.getDataStatus(row)]
...
function presenter (crud) {
  ...
  return {
    ...
    created () {
      for (const key in this.$crud) {
        if (this.$crud.hasOwnProperty(key)) {
          this.$crud[key].toQuery()
        }
      }
    },
    destroyed () {
      for (const key in this.$crud) {
        this.$crud[key].unregisterVM('presenter', this)
      }
    },
    mounted () {
